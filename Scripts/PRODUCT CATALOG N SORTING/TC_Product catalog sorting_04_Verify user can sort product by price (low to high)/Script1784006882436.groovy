...
List<WebElement> priceElements = WebUI.findWebElements(findTestObject('Catalog/products_price'), 10)

List<Double> actualPrices = []
for (WebElement element : priceElements) {
    String priceText = element.getText().replace('$', '')  // hapus simbol $
    actualPrices.add(Double.parseDouble(priceText))
}

List<Double> expectedSortedPrices = new ArrayList<>(actualPrices)
Collections.sort(expectedSortedPrices)  // untuk low to high

WebUI.verifyEqual(actualPrices, expectedSortedPrices)
